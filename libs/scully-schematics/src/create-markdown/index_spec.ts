  ...
  describe('when using the default options', () => {
    beforeEach(async () => {
      ...
      appTree = await customRunner
        .runSchematicAsync('md', options, appTree)
        .toPromise();
    });
...
exports.config = {  projectRoot: "./src/app",
  routes: {
    ...
    it(`should setup a new angular module from template`, () => {
      ...
      expect(appTree.files).toContain(
        '/src/app/foo-bar-baz/foo-bar-baz.component.css'
      );
      expect(appTree.files).toContain(
        '/src/app/foo-bar-baz/foo-bar-baz.component.html'
      );
      expect(appTree.files).toContain(
        '/src/app/foo-bar-baz/foo-bar-baz.component.spec.ts'
      );
      expect(appTree.files).toContain(
        '/src/app/foo-bar-baz/foo-bar-baz.component.ts'
      );
      expect(appTree.files).toContain(
        '/src/app/foo-bar-baz/foo-bar-baz.module.ts'
      );
...
describe('create-markdown', () => {
  ...
  describe('when using a specific style file format', () => {
    ...
    it(`should create a file with the expected file extension`, () => {
      expect(appTree.files).toContain(
        '/src/app/blog/blog.component.' + styleFileFormat
      );
    });
    ...
    it(`should create a component using the right style file format`, () => {
      ...
      const componentFileContent = getFileContent(
        appTree,
