...
export async function readFileAndCheckPrePublishSlug(file) {
  ...
  const createSlug = () =>
    `${prependString}${Date.now().toString(36)}_${randomString(32)}`;
  const originalFile = readFileSync(file, 'utf-8');
  const {
    attributes: meta,
    body: fileContent
  }: { attributes: ContentMetaData; body: string } = fm(originalFile);
  ...
  if (meta.hasOwnProperty('published') && meta.published === false) {
    ...
    let unPublishedSlug = slugs.find((sl: string) =>
      sl.startsWith(prependString)
    );
    if (!unPublishedSlug) {
